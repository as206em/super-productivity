...
Super Productivity supports short syntax forms for quickly adding metadata to tasks: tags (`#`), projects (`+`), time estimates (`m`/`h`/`d`), due dates (`@`), value (`-v`), and effort (`-e`). Tags, projects, time estimates, and due dates can be enabled or disabled individually in [[3.02-Settings-and-Preferences#global-settings.Short-Syntax]].
...
### Value And Effort Syntax

- **Symbols:** `-v` for value, `-e` for effort.
- **Purpose:** Set the task score inputs used by score sorting and the Priority page.
- **Grammar:** `-v <level>` and `-e <level>`. A space is required after `-v` or `-e`.
- **Parameters:** `xhigh`, `high`, `mid`, or `low`.
- **Valid:** `"Task -v high -e low"`, `"Task -e xhigh -v mid"`.
- **Score:** Score is calculated from value and effort when both are present. Tasks missing either field sort after tasks with complete score data and are not shown on the Priority page.

## Formal Grammar and Regex Patterns

The parser uses regular expressions for each syntax form. Time: optional `t` prefix, digits and decimal with `m`/`h`/`d`, optional `/` for spent/estimate. Project: `+` with lookahead so delimiters (`#`, `@`, time-like tokens) inside the project name do not terminate the match. Tag: `#` followed by characters until a special character or whitespace. Due: `@` followed by characters until a special character. Value and effort: `-v` or `-e`, followed by one of the supported levels. Exact patterns are defined in `short-syntax.ts`.
...
1. **Order of processing:** Due → Project → Tag → Value/Effort (time is processed together with due).
