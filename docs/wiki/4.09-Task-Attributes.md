...
- **Value and effort** — Optional planning fields used to calculate a task score. Levels are `xhigh`, `high`, `mid`, and `low`; tasks need both fields to appear on the Priority page or participate fully in score sorting.
...
- **Priority scoring** — Value and effort can be assigned from the task context menu or with short syntax (`-v high -e low`). The score is calculated from both fields and is used by score sorting and the Priority page.
