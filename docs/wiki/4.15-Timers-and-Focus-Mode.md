...
- **Task tracking → Focus** — When you **start** tracking a task, the app can start or resume a focus session and open the Focus Mode overlay (if you’re on the main focus screen and no session is active, or if a work session was paused). When you’re on a break and you start tracking a task, the app can skip the break and start work so the session aligns with the task.
- **Task estimates → Focus duration** — If a focus session auto-starts from a tracked task that has an estimate, the session uses the task's remaining estimated time. Tasks without estimates use the normal Focus Mode duration.
