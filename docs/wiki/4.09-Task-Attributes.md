...
- **Section context** — A task can appear in a project or Today section. Section membership is stored on the section, but the task row can show the current section as contextual metadata.
- **Sprint membership** — A task can be part of the current or next sprint. Sprint membership is stored in the sprint lists, but the task row can show it as contextual metadata.
...
- **Value and effort** — Optional planning fields used to calculate a task score. Levels are `xhigh`, `high`, `mid`, and `low`; tasks need both fields to appear on the Priority page or participate fully in score sorting. Project value, section value, task dates, project deadlines, and section deadlines can adjust the score when present.
...
- **Context chips** — Task rows can show the task's project, current section, current or next sprint, and regular tags together below the title so the surrounding work context stays visible while scanning a list.
- **Priority scoring** — Value and effort can be assigned from the task context menu or with short syntax (`-v high -e low`). The score is calculated from both fields, adjusted by optional project/section value and date context, and used by score sorting and the Priority page.
