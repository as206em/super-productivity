...
A full backup or export includes all application data: tasks (active and archived, including value and effort score fields), projects (including value and deadline metadata), sections (including value and deadline metadata), tags, sprint lists, time-tracking state, global configuration (including capacity settings), notes, reminders, metrics, simple counters, planner and board state, task repeat configurations, issue-provider settings, plugin user data, and both archive tiers (recent and old). No data is excluded from a complete backup or export.
