...
- `getAllProjects()`, `addProject(projectData)`, `updateProject(projectId, updates)`. Project data includes optional priority metadata fields `value` (`xhigh`, `high`, `mid`, `low`, or `null`) and `deadlineDay` (`YYYY-MM-DD` or `null`).
